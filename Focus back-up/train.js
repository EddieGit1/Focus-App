...
async function calculateAccuracy() {
    ...
    const tempClassifier = knnClassifier.create();
    trainData.forEach(sample => {
        const tensor = tf.tensor1d(sample.pose);
        tempClassifier.addExample(tensor, sample.label);
    });
    ...
    for (let sample of testData) {
        const tensor = tf.tensor1d(sample.pose);
        const prediction = await tempClassifier.predictClass(tensor);
